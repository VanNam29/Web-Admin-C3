...
import http from "./redux/http-category";

import { ButtonAdd } from "../../components/button-add/button-add";
import Pagination from "@material-ui/lab/Pagination";
import { findCategoryByName } from "./redux/category.service";
import { fetchCategorySuccess } from "./redux/category.action";
...
export const ListCategory: FC = () => {
  ...
  const findByName = (name: string): void => {
    console.log(name);
    setSearchTitle(name);
    const action = findCategoryByName(name);
    dispatch(action);
  };
        ...
        <div className="w-96/100 h-full bg-white m-auto  rounded-4 shadow-md mobile:mt-4 tablet:mt-24 laptop:mt-42">
          ...
          >
            <thead className="shadow">
              <tr className="bg-white mobile:h-32 tablet:h-42 mobile:text-8 tablet:text-16 laptop:text-18 laptop:h-48">
                ...
                <th className="border border-gray-200 w-7/10 text-left mobile:pl-4 tablet:pl-12">
                  Description
                </th>
                <th className="border border-gray-200 w-1/20 text-left mobile:pl-4 tablet:pl-12">
                  Option
          ...
          <div>
            <ModalCustom selector="#portal" isOpen={modalIsOpen}>
              <ModalCategory closeModal={closeModal} />
            </ModalCustom>
          </div>
        </div>
      </div>
...
export const getStaticProps = async () => {
  // const dispatch = useDispatch();
  // const customers: Customer[] = useSelector((state: any) => state.customers.list);
  // useEffect(()=> {
